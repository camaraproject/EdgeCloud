...
Scenario: Successfully register application endpoint with IPv6 address
    Given the path "/application-endpoints"
    And the header "x-correlator" is set to a valid correlation identifier
    And the request body is an array containing one ApplicationEndpointInfo with IPv6 address and required fields
    When the request "registerApplicationEndpoints" is sent
    Then the response status code is 200
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has same value as the request header "x-correlator"
...
Scenario: Fail to register with invalid UUID and IP address formats
    Given the path "/application-endpoints"
    And the header "x-correlator" is set to a valid correlation identifier
    And the request body is an array containing ApplicationEndpointInfo with invalid "edgeCloudZoneId" format and invalid IPv4 address
    When the request "registerApplicationEndpoints" is sent
    Then the response status code is 400
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has same value as the request header "x-correlator"
...
Scenario: Fail to register with ApplicationEndpoint not meeting anyOf constraint
    Given the path "/application-endpoints"
    And the header "x-correlator" is set to a valid correlation identifier
    And the request body is an array containing ApplicationEndpointInfo with applicationEndpoint missing required anyOf combinations
    When the request "registerApplicationEndpoints" is sent
    Then the response status code is 400
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has same value as the request header "x-correlator"
...
@app_endpoint_registration_11_invalid_id_format
Scenario: Fail to retrieve endpoint with invalid ID format
    Given the path "/application-endpoints/{applicationEndpointsId}"
...
@app_endpoint_registration_12_no_auth_token
...
@app_endpoint_registration_13_insufficient_scope
...
@app_endpoint_registration_14_invalid_token_context
...
@app_endpoint_registration_15_endpoint_not_found
...
@app_endpoint_registration_16_unidentifiable_application_profile
Scenario: Fail when application profile cannot be identified from token or request
    Given the path "/application-endpoints"
    And the header "x-correlator" is set to a valid correlation identifier
    And an API consumer with a token that doesn't contain application profile information
    And the request body is an array containing ApplicationEndpointInfo without applicationProfileId
    When the request "registerApplicationEndpoints" is sent
...
@app_endpoint_registration_17_service_not_supported
...
@app_endpoint_registration_18_register_multiple_endpoints
...
@app_endpoint_registration_19_complete_crud_flow
Scenario: Complete flow - register, verify, update, and delete endpoints
    ...
    When I retrieve the endpoint using the stored ID
    Then the response status code is 200
    And the retrieved endpoint matches the originally registered data
    
    When I update the endpoint using the stored ID
    Then the response status code is 204
    
    When I delete the endpoint using the stored ID
    Then the response status code is 200
    
    When I attempt to retrieve the deleted endpoint
    Then the response status code is 404

@app_endpoint_registration_20_x_correlator_validation
Scenario: Validate x-correlator header handling across all operations
    Given the path "/application-endpoints"
    And the header "x-correlator" is set to a unique correlation identifier
    And the request body is an array containing valid ApplicationEndpointInfo
    When the request "registerApplicationEndpoints" is sent
    Then the response status code is 200
    And the response header "x-correlator" has same value as the request header "x-correlator"
    And the correlation identifier enables request-response tracking across operations
