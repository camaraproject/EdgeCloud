...
@EdgeCloud_LCM_app_submission_ok
...
@EdgeCloud_LCM_app_submission_conflict
Scenario: Submit an Application with mandatory parameters
Given an application with the same name and version had already been submitted
When invoking with the POST method to submit an app with all required parameters
...
Given an application has already been submitted
When invoking with the POST method to instantiate an app with mandatory parameters ("appId" in path, and "region" in body) and optional parameter ("EdgeCloudZone")
Then response code is 202
...
Scenario: Instantiate an Application with mandatory parameters ("appId" in path, and "region" in body) and optional parameter ("EdgeCloudZone")
Given an application has already been submitted, but already instantiated in given EdgeCloudZone
When invoking with the POST method to instantiate an app with mandatory parameters ("appId" in path, and "region" in body) and optional parameter ("EdgeCloudZone")
Then response code is 409
...
@EdgeCloud_LCM_app_instance_delete_all_instances
Scenario: Delete all running instances of an application in Edge Cloud
When invoking with the DELETE method to terminate running instances of an application within Edge Cloud with just mandatory parameter ("appId")
Then response code is 202
And all the application instances termination initiated

@EdgeCloud_LCM_app_instance_delete_valid_parameter
...
@EdgeCloud_LCM_app_instance_delete_invalid_parameter
Scenario: Delete a running instance of an application within an Edge Cloud Zone with optional parameter ("appInstanceId")
...
Scenario: Get information of existing Edge Cloud Zones with optional parameters ("region","status")
Given region provided does not exist
When invoking the GET method to obtain information of Edge Cloud Zones with a region and a status (active,inactive,unknown)
Then response code is 404
